...
impl<S: ChainSource> Attempt<'_, S> {
    ...
    async fn resolve(&self) -> Result<Resolved> {
        ...
        // ceiling is requested here. A resolver that is not given one runs
        // out of compute instead of reporting its work.
        let (tx, _) = self
            .turner
            .signed_tx(&[resolver_ix], self.turner.tx_config(MAX_COMPUTE_UNITS, 0))
            .await?;
        let sim = self
            .turner
            .source()
    ...
    async fn simulate(&self, ixs: &[Instruction]) -> Result<SimOutcome> {
        let (tx, _) = self
            .turner
            .signed_tx(ixs, self.turner.tx_config(MAX_COMPUTE_UNITS, 0))
            .await?;
