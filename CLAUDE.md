...
- **Signer status is transaction-global.** Marking an executor's `AccountMeta` `is_signer: false` does NOT stop the runtime reporting it as a signer if it signs the transaction. The payout account must therefore be separate from the fee payer, and untrusted executors must never name the fee payer (`names_signer` enforces this). Never relax either without understanding that an executor can CPI a System transfer with any account that signs.
